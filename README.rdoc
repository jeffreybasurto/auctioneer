...
later in app once you're placing an item you only need to select the correct House.
...
examples of querying for auctions.
  @auctions.find "all"
  @auctions.find "all.sword"  # all auctions that match for "sword"
  @auctions.find "/sword/" # first auction that matches the regular expression.
  @auctions.find "x2.black.sword"  # first two auctions that match black and then match sword.
  @auctions.find "#10"  # auction with ID ticket of 10
  @auctions.find "#10 and #25"  # auctions with ID ticket 10 and 25

To render all auctions (via rails partial syntax) you might do something like this:
  render :partial => 'auction', :collection => @auctions.find "all"
